...
![未处理、HD-2D 与 ESPCN 对比](pictures/comparison.png)

同一帧，掌机实拍，60 FPS。差别在下排才看得清：不处理时画面是双线性糊开的。
**HD-2D** 先把作者画的斜线重建回来，再用估出的深度重新打光，而**一个像素都不改**。
**ESPCN 6x** 按屏幕实际绘制的倍率重建亮度。
...
| **HD-2D 光影** | 想要"现代化"观感的 2D 像素画 | +1 帧（深度） |
