...
8. **改完必须让天马G 重扫**，见 §5.2 第 4 步——魔改版不重扫就永远不生效，而这个失败**在日志里长得跟"改写没生效"一模一样**。
   ...
   > **这条决定了面向玩家的流程，不只是我们调试时的注意事项。** 覆写是静默的、每次开 APP 都跑；不说出来的话，APP 会显示"22/22 已设置"，而玩家点开游戏发现毫无变化，任何地方都不给理由——**就是坑了我们一整天的那个失败，只不过瞄准了玩家**。所以：**改动发生的那一刻就弹窗说明**，而不是等玩家去发现。
   >
   > **需要两次重新加载，而且是结构性的，压不成一次**：复制只在 RA 加载 shim 时发生，而要 RA 加载 shim，至少得有一条启动项已经指向它。
   >
   > | 玩家 | 背后 |
   > |---|---|
   > | 按引导装那一个核心 | — |
   > | 回 APP | 改写 **1** 条（只有它的 shim 存在） |
   > | **天马G 重新加载** | 这条生效 |
   > | 玩一次那个机种 | shim 加载 → 复制出其余 21 个 |
   > | 回 APP | 改写剩下 **21** 条 |
   > | **天马G 重新加载** | 全部生效，之后永不再管 |
   >
   > **不要为了省一次重载而乐观地一次全改写。** 那意味着在 shim 首次加载之前，所有指向不存在 shim 的机种**全都打不开**——"你得先玩一次 GBA，MD 游戏才能开"是不可接受的故障。§5.4 第 7 条那个联锁就是为这个存在的。
